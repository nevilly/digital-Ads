...
class Podcasts extends PodcastsController
{
    ...
    public function add(){
        ...
        $pod = isset($data->pod) ? $this->clean->clean_input($data->pod) : '';
        ...
        if(!empty($title) && !empty($file) && !empty($pod)){
            ...
            if(!parent::checkPodcast($title)){
                $sql = parent::addPodcast(parent::encode(array("title"=>$title,"pod"=>$pod,"description"=>$desc,'user'=>$user_id,"file"=>$file)));
    ...
    public function update($a ='')
    {
        ...
        if(!empty($title) && !empty($category)
            ...
            if($sql){
                $status = 200;
                $data = 'Podcast was update successful!';
            }
        }
    ...
    public function remove($params){
        ...
        if(!empty($user_id) && !empty($this->data->id)) {
            if(parent::deletePodcast($this->data->id, $user_id)){
                $status = 200;
                $data = "Podcast was successful deleted!";
            }
        }
        return array('status'=>$status, 'payload'=>$data);
    }
