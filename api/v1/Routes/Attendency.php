...
class Attendency extends EmployeeAttendencyController
{
    private $data;
    private $clean;
    private $key;
    ...
    public function add(){
     ...
     // if($this->clean->Method() !== "POST" && !(array) $this->data || !Util::tokenValidate())
        ...
        $MonthlyWage = isset($data->checkOut) ? $this->clean->clean_input($data->MonthlyWage) :'';
    ...
    public function save_data(){
        ...
        $MonthlyWage = isset($data->MonthlyWage) ? $this->clean->clean_input($data->MonthlyWage) :'';
             ...
             $netPay = $deduction == '0' ? '0' : (int)$MonthlyWage - (int)$deduction;
    ...
    public function add_department(){
        ...
        if(!empty($data->name))
            if(parent::add_Edepartment(parent::encode(["name"=>$data->name]))){
                $status = 200;
    ...
    public function remove($params){
        ...
        if(!empty($this->data->uid)) {
            if(parent::delete_empAttndcy($this->data->uid,'')){
                $status = 200;
                $data = "Employee was successful removed!";
            }
        }
        return array('status'=>$status, 'payload'=>$data);
    }
